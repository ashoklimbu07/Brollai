...
type VisualPromptSpec = {
  ...
  text_rules?: {
    ...
    logo_policy: 'no logos';
    text_policy: 'no on-screen text';
    contrast: string;
  };
  color_palette?: {
    background: string;
...
type ScriptAnalysisResponse = {
  scenes: Array<{
    originalText: string;
    visualPrompt: VisualPromptSpec | string;
  }>;
};
...
const getVideoAnalyzerApiKeyConfig = (): { key: string; keyName: string } => {
  ...
  throw new Error(
    'Missing Gemini API key for Video Scene Analyzer. Set GEMINI_API_KEY_VIDEO_SCENE_ANALYZER or ANALYZER_GEMINI_KEY in backend/.env',
  );
};

const normalizeVisualPromptForClient = (raw: unknown): string => {
  if (raw == null) {
    return '{}';
  }
...
const mapResponseToScenes = (json: ScriptAnalysisResponse) => {
  return json.scenes.map((scene, index) => ({
    id: `scene-${Date.now()}-${index}`,
    originalText: scene.originalText,
    visualPrompt: normalizeVisualPromptForClient(scene.visualPrompt as unknown),
  ...
  try {
    ...
    if (Array.isArray(jsonPrompt.timeline)) {
      const actions = jsonPrompt.timeline
        .filter((point: { action?: string }) => point.action && point.action !== 'SWAP_ME')
        .map((point: { action?: string }) => point.action)
        .join(', ');
      if (actions) {
...
export const videoSceneAnalyzerService = {
  async analyzeVideo(videoBase64: string, mimeType: string) {
    ...
    console.log(
      `[VideoSceneAnalyzer] Processing video analyze | keyName=${keyName} | key=${maskApiKey(key)} | mimeType=${mimeType} | base64Length=${videoBase64.length}`,
    );
  ...
  async regenerateVisualPrompt(segmentText: string) {
    ...
    const json = JSON.parse(response.text || '{}') as { visualPrompt?: VisualPromptSpec | string };
    return normalizeVisualPromptForClient(json.visualPrompt ?? response.text);
  },

  async generateImage(prompt: string, styleModifier: string, aspectRatio: string) {
  ...
  async generateVideo(prompt: string, styleModifier: string, aspectRatio: string) {
    const { key } = getVideoAnalyzerApiKeyConfig();
    const client = new GoogleGenAI({ apiKey: key });
    const fullPrompt = parseVisualPromptToText(prompt, styleModifier);
    ...
    const responseData = operation.response || (operation as { result?: { generatedVideos?: Array<{ video?: { uri?: string } }> } }).result;
