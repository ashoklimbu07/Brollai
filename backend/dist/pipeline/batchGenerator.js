...
function parseBatchScenes(rawText) {
    try {
        ...
        return objectBlocks.map((block) => safeParseJSON(block));
    }
}
        ...
        try {
            ...
            for (let attempt = 1; attempt <= CONFIG.MAX_RETRIES; attempt += 1) {
                try {
                    ...
                    console.log(`[B-roll] Batch ${batchLabel}${attemptNote} · scenes ${firstSceneId}–${lastSceneId} · key #${keyEntry.id} …${keyLast4}`);
                    const prompt = params.style === '2d_animation'
                    ...
                    if (parsed.length !== batchSceneTotal) {
                        throw new Error(`Batch ${batchLabel} returned ${parsed.length} scenes, expected ${batchSceneTotal}`);
                    }
