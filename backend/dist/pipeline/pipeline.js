...
export async function runPipeline(script, sceneCount, style, signal) {
    ...
    console.log(`Pipeline key status -> analyzerConfigured=${analyzerConfigured} analyzerKeyLast4=${analyzerLast4} pooledKeys=${CONFIG.API_KEYS.length}`);
    console.log(`[B-roll] Master analyzer using dedicated key ...${analyzerLast4}`);
    if (CONFIG.API_KEYS.length !== 5) {
        throw new Error(`Exactly 5 API keys are required (found ${CONFIG.API_KEYS.length}). Set GEMINI_KEY_1..5 or GEMINI_API_KEY_BROLL1..5 in backend/.env.`);
    }
